...
<script>
    ...
    // สร้างอาเรย์สำหรับ labels และ data โดยรวมยอดขายของร้านค้าที่ซ้ำกัน
    const labels = [...new Set(salesData.map(sale => sale.shop.name ?? 'ไม่พบข้อมูล'))];
    const data = labels.map(label => {
        return salesData
            .filter(sale => sale.shop.name === label)
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: [
                    ...
                    'rgba(75, 192, 192, 0.2)',
                    'rgba(153, 102, 255, 0.2)',
                    // เพิ่มสีสำหรับร้านค้าเพิ่มเติมตามต้องการ
                ],
                borderColor: [
                    'rgba(75, 192, 192, 1)',
                    'rgba(153, 102, 255, 1)',
